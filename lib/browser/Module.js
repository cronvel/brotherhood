...
Package.prepare = ( id , mainModuleId ) => {
...
Module.prepare = ( id , packageId , fn ) => {
...
Module.require = ( path , base = '/' ) => {
	...
	if ( ! id ) {
		...
		if ( ! Module.require.resolveCache[ base ] ) { Module.require.resolveCache[ base ] = {} ; }
...
Module.require.resolve = ( initialPath , initialBase = '/' ) => {
	...
	if ( packageName ) {
		...
		if ( packageName.startsWith( 'core:' ) ) {
			...
			found = Module.recursivePackageSearch( packageName , path , '/[core]/node_modules' , '/[core]/node_modules' ) ;
		}
		else {
